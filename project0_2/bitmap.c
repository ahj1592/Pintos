...
struct bitmap* bitmap_expand(struct bitmap *bitmap, int size){
	size_t pre_size = bitmap_size(bitmap);
	size_t new_size = pre_size + (size_t)size; /* re-size the bitmap */
	
	/* new bitmap to be returned */
	struct bitmap *nb = bitmap_create(new_size);

	/* copy the bitmap, expanded bits are setted false */
	for(size_t i = 0; i < pre_size; i++){
		bitmap_set(nb, i, bitmap_test(bitmap, i));
	}
	for(size_t i = pre_size; i < new_size; i++){
		bitmap_set(nb, i, false);
	}
