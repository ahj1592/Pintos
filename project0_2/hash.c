...
/* my hash_hash_func */
unsigned my_hash_func(const struct hash_elem *he, void *aux){
...
/* my hash_less_func */
bool my_hash_less (const struct hash_elem *a,
		const struct hash_elem *b,
		void *aux)
{
	struct hash_item *ta = hash_entry(a, struct hash_item, helem);
	struct hash_item *tb = hash_entry(b, struct hash_item, helem);

	return ta->key < tb->key;
}

/* my hash_action_func : destructor, square, triple */

/* destroy HASH_ELEM, which means deallocate memory of HASH_ELEM */
void destructor (struct hash_elem *e, void *aux){
	free(e);
}

void square(struct hash_elem *e, void *aux){
	struct hash_item *item = hash_entry(e, struct hash_item, helem);
...
/* Additionaly implemented hash function */
unsigned
hash_int_2(int i){
	/* use Mersenne primes(2^t - 1) */
	int p = 1;
